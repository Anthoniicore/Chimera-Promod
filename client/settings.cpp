        ...
        while(std::getline(init, line))
        {
            ...
            switch(execute_chimera_command(line2.data(), !verbose_init, true))
            {
                case CHIMERA_COMMAND_ERROR_TOO_MANY_ARGUMENTS:
                {
                    ...
                    sprintf(x, "%s:%u: Function %s takes no more than %u arguments.", name, ln, command.name(), command.max_args());
                    console_out_error(x);
                    break;
                }
                ...
                {
                    auto &command = find_chimera_command(line.data());
                    sprintf(x, "%s:%u: Function %s takes at least %u arguments.", name, ln, command.name(), command.min_args());
                    console_out_error(x);
                    break;
                }
