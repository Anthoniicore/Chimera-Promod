...
namespace Chimera {
    namespace { OpusEncoder *g_encoder = nullptr; OpusDecoder *g_decoder = nullptr; }
    ...
    bool initialize_voice_codec() noexcept {
        ...
        if(!g_encoder) {
            ...
            if(!encoder || error != OPUS_OK) { if(encoder) opus_encoder_destroy(encoder); return false; }
        ...
        if(!g_decoder) {
            ...
            if(!decoder || error != OPUS_OK) { if(decoder) opus_decoder_destroy(decoder); return false; }
            g_decoder = decoder;
        }
        return true;
    }
    ...
    bool encode_voice_audio_packet(const int16_t *pcm, size_t samples, std::vector<uint8_t> &packet) noexcept {
        ...
        const int encoded = opus_encode(g_encoder, reinterpret_cast<const opus_int16 *>(pcm), static_cast<int>(samples), packet.data(), static_cast<opus_int32>(packet.size()));
    ...
    bool decode_voice_audio_packet(const uint8_t *packet, size_t packet_size, std::vector<int16_t> &pcm) noexcept {
        ...
        const int decoded = opus_decode(g_decoder, packet, static_cast<opus_int32>(packet_size), reinterpret_cast<opus_int16 *>(pcm.data()), static_cast<int>(VOICE_OPUS_FRAME_SAMPLES), 0);
        if(decoded <= 0) { pcm.clear(); return false; }
        pcm.resize(static_cast<size_t>(decoded) * VOICE_AUDIO_CHANNELS);
        return true;
    }
}
