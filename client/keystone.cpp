...
static void handle_keystone_text(const short *text) {
    char b[128] = {};
    for(int i=0;i<static_cast<int>(sizeof(b))-1;i++)
    {
        if(text[i] == 0) break; // stop at the terminator (a switch-break would only exit the switch)
        b[i] = static_cast<char>(text[i]);
    }
    console_out(b);
}
