...
struct OffsetHudMod {
    ...
    char objects[65536] = {}; // indexed by uint16_t tag index; must hold 0..65535 (0xFFFF = null id)
...
static void do_things_to_weap_tag(HaloTagID tag_id, OffsetHudMod &mod) noexcept {
    if(tag_id.is_valid() && !mod.objects[tag_id.index] && *reinterpret_cast<uint16_t *>(HaloTag::from_id(tag_id).data) == 2)
    {
        mod.objects[tag_id.index] = 1;
