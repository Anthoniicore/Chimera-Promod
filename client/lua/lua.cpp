...
bool check_for_it(const char* script_data, size_t data_size) noexcept
{
  ...
  // Search for patterns. Use additive bounds (i + size <= data_size) rather than
  // (data_size - size), which would underflow size_t for buffers smaller than the
  // pattern and scan ~4 GB out of bounds.
  for (size_t i = 0; i + sizeof(gdp_pattern) <= data_size; i++)
  {
    if (memcmp(script_data + i, gdp_pattern, sizeof(gdp_pattern)) == 0)
    {
      ...
      for (size_t j = i; j + sizeof(sequence2) <= data_size; j++)
      {
