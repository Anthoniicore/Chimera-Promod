...
// Lerp lineal puro para el arma en primera persona.
// Sin Hermite, sin buffer de historial extra — respuesta directa.
...
void fp_before() noexcept {
    ...
    if(tick_now != tick_before) {
        ...
        uint32_t current_id;
        if(odata) current_id = *(reinterpret_cast<uint32_t *>(odata + 0x2F8)
                              + *reinterpret_cast<uint16_t *>(odata + 0x2F2));
        else current_id = NULL_ID;

        bool weapon_changed = (current_id != weapon_id);
        skip = get_camera_type() != CAMERA_FIRST_PERSON || weapon_changed;
    ...
    for(int i = 0; i < 128; i++) {
        // Rotación: slerp
        interpolate_quat(
            fpbuffer1[i].rotation_stuff,
            fpbuffer0[i].rotation_stuff,
            fpn[i].rotation_stuff, tp);

        // Posición: lerp lineal
        interpolate_vector(
            fpbuffer1[i].position,
            fpbuffer0[i].position,
            fpn[i].position, tp);

        // Escala: lerp
        fpn[i].unknown_1 = fpbuffer1[i].unknown_1
            + (fpbuffer0[i].unknown_1 - fpbuffer1[i].unknown_1) * tp;
    }
}
