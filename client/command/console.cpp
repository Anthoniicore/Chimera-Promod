...
static void block_error() noexcept {
    ...
    // The bytes actually written are the 5-byte call at offsets +6..+10, so the
    // protected range must start at push_req + 6 (not push_req) to be correct
    // when the signature sits near a page boundary.
    if(VirtualProtect(push_req + 6, 5, PAGE_READWRITE, &old_protect) == false) return;
    for(int i=0;i<5;i++) *(push_req + 6 + i) = 0x90;
    VirtualProtect(push_req + 6, 5, old_protect, &old_protect_b);
}
