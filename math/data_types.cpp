...
void interpolate_vector_rotation(const Vector3D &before, const Vector3D &after, Vector3D &output, float scale) noexcept {
    ...
    float yrs = fabs(yr);
    float zrs = fabs(zr);
    if(yrs > xrs && yrs >= zrs) highest = 1;      // true argmax over {x,y,z}
    else if(zrs > xrs && zrs >= yrs) highest = 2;
    else highest = 0;
...
//
// Fórmula estándar de Hermite:
//   h00(t) = 2t³ - 3t²  + 1   (peso para p0)
//   h10(t) =  t³ - 2t²  + t   (peso para t0)
//   h01(t) = -2t³ + 3t²        (peso para p1)
//   h11(t) =  t³ - t²          (peso para t1)
//
// Las tangentes t0 y t1 son la velocidad estimada en cada extremo.
// Se calculan en camera.cpp como (p_next - p_prev) * 0.5 (tangente de Catmull-Rom).
//
void interpolate_vector_hermite(
    const Vector3D &p0, const Vector3D &p1,
    const Vector3D &t0, const Vector3D &t1,
    Vector3D &output, float t) noexcept
{
    float t2 = t  * t;
    float t3 = t2 * t;
    ...
    output.x = h00*p0.x + h10*t0.x + h01*p1.x + h11*t1.x;
    output.y = h00*p0.y + h10*t0.y + h01*p1.y + h11*t1.y;
    output.z = h00*p0.z + h10*t0.z + h01*p1.z + h11*t1.z;
}

void interpolate_vector_rotation_hermite(
    const Vector3D &p0, const Vector3D &p1,
    const Vector3D &t0, const Vector3D &t1,
    Vector3D &output, float t) noexcept
{
    interpolate_vector_hermite(p0, p1, t0, t1, output, t);

    // Re-normalizar para mantener el vector como dirección unitaria
    float len = sqrt(output.x*output.x + output.y*output.y + output.z*output.z);
    if(len > 0.0001f) {
        output.x /= len;
        output.y /= len;
        output.z /= len;
